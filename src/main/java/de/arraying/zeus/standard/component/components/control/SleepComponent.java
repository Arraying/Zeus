 ...
 */
public class SleepComponent implements ZeusStandardComponent {
    ...
    public boolean invoke(ZeusTaskImpl task, Token[] tokens, int lineNumber)
        ...
        if(tokens.length != 2) {
            throw new ZeusException("Expected just the sleep keyword and a duration (long), nothing more/less.", lineNumber);
        }
        ...
        long sleep;
        try {
            sleep = Long.valueOf(duration.getToken().substring(0, duration.getToken().length()-1));
        ...
        task.onEvent(new SleepEvent(sleep, lineNumber));
        try {
            Thread.sleep(sleep);
